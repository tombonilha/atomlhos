...
class AtomalhosApp {
  ...
  async getWebsitePreview(url) {
    try {
      console.log(`Attempting to get preview for: ${url}`);
      
      // Usar a API Microlink com parâmetros otimizados
      const microlinkApiUrl = `https://api.microlink.io?url=${encodeURIComponent(url)}&screenshot=true&meta=false&embed=screenshot.url&viewport.width=1200&viewport.height=800&screenshot.type=png&screenshot.quality=80`;
      
      const response = await fetch(microlinkApiUrl);
      
      if (!response.ok) {
        console.error(`Microlink API returned status: ${response.status}`);
      ...
      if (data.status === "success" && data.data) {
        ...
        console.log("Preview data extracted:", previewData);
        return previewData;
      } else {
        console.log("Microlink API did not return success status or data.", data);
